...
struct portmap_metadata_t {
    ...
    bit<NUM_SPINE_DS> common_portmap;
}

struct link_metadata_t {
    bit<8> link_idx;
    bit<32> link_bitstring;
}
...
// At ToR: if leaf_status_bit == 1, extract leaf_us label
...
header src_label_t {
    ...
    bit<NUM_SPINE_DS> spine_common_ds;
    bit<32> spine_ds; 
